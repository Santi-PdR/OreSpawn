...
/** Port directo de danger.orespawn.entity.VelocityRaptor del JAR 1.12.2 proporcionado. */
public final class VelocityRaptorEntity extends TamableAnimal {
    private static final double MOVE_SPEED = 0.55D;

    public VelocityRaptorEntity(EntityType<? extends VelocityRaptorEntity> type, Level level) {
        super(type, level);
        this.xpReward = 5;
    }
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(2, new FollowOwnerGoal(this, 1.5D, 10.0F, 2.0F, false));
        goalSelector.addGoal(4, new TemptGoal(this, 1.25D, Ingredient.of(Items.APPLE), false));
        goalSelector.addGoal(5, new PanicGoal(this, 1.600000023841858D));
        goalSelector.addGoal(6, new LookAtPlayerGoal(this, Player.class, 6.0F));
        goalSelector.addGoal(7, new LongRangeWanderGoal(this, 10, 0.9D));
    ...
    private BlockPos findNearestEdiblePlant() {
        ...
        for (int radius = 1; radius < 10; radius += radius >= 5 ? 2 : 1) {
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (isTame() && isOwnedBy(player) && stack.is(Blocks.DEAD_BUSH.asItem())
                && player.distanceToSqr(this) < 16.0D) {
            if (!level().isClientSide) {
                setTame(false);
                setOwnerUUID(null);
                setHealth(10.0F);
        ...
        if (isTame() && isOwnedBy(player) && stack.is(Items.NAME_TAG)
                && player.distanceToSqr(this) < 16.0D) {
            if (!level().isClientSide) {
    ...
    @Nullable
    @Override
    public VelocityRaptorEntity getBreedOffspring(ServerLevel level, AgeableMob mate) {
        return ModEntities.VELOCITY_RAPTOR.get().create(level);
    }
    ...
    public boolean hurt(DamageSource source, float amount) {
        return super.hurt(source, Math.min(10.0F, amount));
    }

    public static boolean checkSpawnRules(EntityType<VelocityRaptorEntity> type, ServerLevelAccessor level,
