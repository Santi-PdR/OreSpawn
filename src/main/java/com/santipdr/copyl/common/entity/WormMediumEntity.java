...
public final class WormMediumEntity extends Monster {
    ...
    public void aiStep() {
        ...
        if (!normalCycle) {
            ...
            downCount = 0;
            if (!isBurrowBlockAllowed(blockAtYOffset(3.0D))) { discard(); return; }
            ...
            --upCount;
            if (upCount == 0) downCount = 100 + random.nextInt(150);
            if (nearbyPlayer != null) pointAt(nearbyPlayer);
            if (!isBurrowBlockAllowed(blockAtYOffset(0.25D))) { discard(); return; }
            ...
            else upCount = 25 + random.nextInt(75);
            if (!isBurrowBlockAllowed(blockAtYOffset(3.0D))) { discard(); return; }
            setDeltaMovement(getDeltaMovement().x, getDeltaMovement().y + 0.10000000149011612D, getDeltaMovement().z);
            setPos(getX(), getY() + 0.05000000074505806D, getZ());
        }
    ...
    private WormSmallEntity nearestSmallWorm(double x, double y, double z) {
        return level().getEntitiesOfClass(WormSmallEntity.class, getBoundingBox().inflate(x, y, z), Entity::isAlive)
                .stream().min(Comparator.comparingDouble(this::distanceToSqr)).orElse(null);
    }
    ...
    private static boolean isBurrowBlockAllowed(BlockState state) {
        return state.isAir() || state.is(Blocks.GRASS) || state.is(Blocks.TALL_GRASS)
                || state.is(Blocks.GRASS_BLOCK) || state.is(Blocks.DIRT) || state.is(Blocks.STONE);
    }
    ...
    @Override public void tick() { if (isPersistenceRequired()) noPhysics = false; super.tick(); setDeltaMovement(getDeltaMovement().x, getDeltaMovement().y * 0.65D, getDeltaMovement().z); }
    ...
    @Override public boolean hurt(DamageSource source, float amount) { return source.is(DamageTypes.IN_WALL) ? false : super.hurt(source, amount); }

    public static boolean checkSpawnRules(EntityType<WormMediumEntity> type, ServerLevelAccessor level, MobSpawnType reason, BlockPos pos, RandomSource random) {
        return !level.getLevel().isDay();
    }

    @Override public boolean removeWhenFarAway(double distanceToClosestPlayer) { return false; }
    @Override protected SoundEvent getAmbientSound() { return null; }
    @Override protected SoundEvent getHurtSound(DamageSource source) { return ModSounds.LITTLE_SPLAT.get(); }
    @Override protected SoundEvent getDeathSound() { return ModSounds.BIG_SPLAT.get(); }
    @Override protected float getSoundVolume() { return 0.5F; }
    @Override public float getVoicePitch() { return 1.5F; }

    @Override
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        super.dropCustomDeathLoot(source, looting, recentlyHit);
        for (int i = 0; i < 2; i++) dropOriginalItem(Items.ROTTEN_FLESH);
        for (int i = 0; i < 2; i++) dropOriginalItem(Items.LEATHER);
    }
